...
function addCustomSection(container){
    ...
    container.appendChild(cardEle)
    cardEle.querySelector("input").addEventListener("change",function(e){
        cardEle.dataset.content = this.value;
    })
...
window.onload = () => {
    ...
    window.addEventListener("partner-draggadd",function(data){
        console.log("adding",data.detail)
        console.log(containers,data.detail.containerID)
        addCard([data.detail.text],containers[data.detail.containerID]);
    })
    ...
    for(let i = 0;i<containers.length;i++){
        ...
        containers[i].addEventListener('dragover', e => {
            e.preventDefault()
            // alert(draggable)
            if(currentDragable.parentNode !== containers[i]){
                window.dispatchEvent(new CustomEvent("addcard",{detail:{containerID:i,text:currentDragable.dataset.content}}))
            }

            containers[i].appendChild(currentDragable)
        })
    ...
    cardContainer.addEventListener('dragover', e => {
      ...
      //  window.dispatchEvent(new CustomEvent("dcard",{detail:{containerID:i,text:currentDragable.dataset.content}}))
    ...
    window.addEventListener("partner-nextform",function(){
    ...
    document.querySelector('#doneBtn').addEventListener('click', (e) => {
        if (formFlowFactor < 3){
            if(e.isTrusted){
            window.dispatchEvent(new Event("nextform"));}
            hideText.play()
            setTimeout(() => {
